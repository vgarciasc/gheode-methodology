...
def create_request(year, resolution, variable_id):
    return {
...
def send_request(args):
    year, resolution, variable_id, output_dir, output_prefix = args

    filepath = os.path.join(output_dir, f"{output_prefix}_g{resolution}_{year}_data.grib")
    if os.path.exists(filepath):
        print(f"Already downloaded '{filepath}'.")
        return

    print(f"Sending request for year {year}.")
    request = create_request(year, resolution, variable_id)
    client.retrieve('reanalysis-era5-single-levels', request, filepath)


def download_variable(start_year, end_year, variable_id, resolution, output_dir, output_prefix, n_jobs):
    years = list(range(start_year, end_year + 1))
    params = [(y, resolution, variable_id, output_dir, output_prefix) for y in years]
...
if __name__ == "__main__":
    ...
    download_variable(args.start_year, args.end_year, args.variable_id, args.resolution,
